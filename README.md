...
- **拼音 / 同音容错检索**：输入 `ximeng` 可命中"西蒙"；错别字"宫崎竣"也能找到"宫崎骏"（基于官方拼音表，零第三方依赖）
- **命中高亮 + 片段预览**：搜索结果的标题/简介高亮关键词，正文显示命中上下文片段
- **组合过滤**：搜索结果可按 分类 / 频道 / 仅付费 / 日期范围 二次筛选（CLI 同支持：`--category --album --paid --from --to`）
- **频道浏览**：Web"📻 频道"页浏览全部 252 个频道（封面/期数/付费标记），点入查看频道全部期数（分页）；CLI 用 `py gcal.py channel <id>`
- **🔥 热榜**：近 7 天播放增长榜（Web 弹窗 + CLI `py gcal.py hot`）；**播放量快照仅在每日定时任务（`daily`）时更新**，Web 刷新按钮/小时级增量不触发，保证热榜按日稳定节奏
...
### 4. 定时抓取与数据守护（Windows 计划任务）
- 每日 **12:00** 执行**"一条龙"**（`py gcal.py daily`）：① 数据库备份（保留最近 7 份，`data/backups/`）→ ② 增量抓取 → ③ 播放量快照 → ④ 数据完整性自检（逐分类比对 API 计数，不一致自动告警）
- 每 6 小时轻量增量（`install_task.ps1` 一键安装，无需管理员）
- 日志自动轮转（5MB × 5 份）；SQLite 连接带写锁等待（busy_timeout），多任务并发安全
...
py gcal.py search 录音笔 --paid 1 --category 会员专享   # 组合过滤
py gcal.py suggest 宫               # 关键词提示
py gcal.py stats                    # 索引统计
py gcal.py incremental              # 每日增量（新期数 + 近30天评论/播放刷新；不含播放快照）
py gcal.py daily                    # 一条龙：备份 + 增量 + 播放快照（热榜数据源） + 完整性自检
py gcal.py backup                   # 备份索引库（保留最近7份）
py gcal.py hot                      # 近 7 天播放增长榜（--days --limit）
py gcal.py channel 51               # 查看频道期数（--page）
